...
const DeliveryScooterIcon = ({ className = "" }) => (
  <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    {/* 뒷쪽 커다란 배달 박스 */}
    <rect x="6" y="18" width="20" height="20" rx="2" fill="currentColor" />
    {/* 스쿠터 바디 라인 */}
    <path d="M26 38 H48 L54 26 H42 L26 38" fill="currentColor" fillOpacity="0.3" stroke="currentColor" strokeWidth="2" />
    {/* 바퀴 */}
    <circle cx="20" cy="48" r="6" stroke="currentColor" strokeWidth="3" />
    <circle cx="48" cy="48" r="6" stroke="currentColor" strokeWidth="3" />
    {/* 라이더 실루엣 (헬멧 포함) */}
    <circle cx="38" cy="16" r="5.5" fill="currentColor" />
    <path d="M32 38 C32 25 38 24 44 38" fill="currentColor" />
    {/* 핸들바 */}
    <path d="M48 38 L52 20 H58" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
export default function Page() {
  ...
  // 핵심 로직: 첫 접속(전체 탭 + 검색어 없음)일 때만 리스트를 비워 푸터가 바로 보이게 함
  const isInitialState = activeTab === '전체' && searchTerm === '';
    ...
    <div className="min-h-screen bg-[#070b14] text-white font-sans tracking-tight pb-40 relative overflow-x-hidden">
      ...
      <div className="bg-[#0f172a]/95 border-b border-slate-800/60 sticky top-0 z-40 backdrop-blur-lg shadow-2xl">
        <div className="p-5 flex items-center gap-4 relative z-10">
          <DeliveryScooterIcon className="w-16 h-16 text-yellow-500 drop-shadow-[0_0_10px_rgba(234,179,8,0.3)]" />
        ...
        <div className="px-5 pb-5 relative z-10">
          <div className="relative group flex items-center">
            <Search className="absolute left-4 text-slate-800 group-focus-within:text-yellow-500 transition-colors pointer-events-none z-20" size={20} />
            <input 
              type="text" 
              placeholder="건물명 초성 검색 가능 (예: ㄱㄹㄷㅂ)" 
              className="w-full p-4.5 pl-12 bg-[#1e293b] rounded-2xl border border-slate-700/50 focus:border-yellow-500/60 text-lg focus:outline-none placeholder:text-slate-800 shadow-xl transition-all font-bold" 
              value={searchTerm}
      ...
      <footer className={`flex flex-col items-center text-center px-10 transition-all duration-700 relative z-10 ${isInitialState ? 'mt-20 opacity-100' : 'mt-10 opacity-30 scale-95'}`}>
        <div className="w-full max-w-sm bg-[#1e293b]/60 backdrop-blur-md p-8 rounded-[3.5rem] border border-slate-800 shadow-2xl flex flex-col items-center gap-6">
          <SealIcon className="w-20 h-20 shadow-2xl shadow-red-900/40" />

...
// [아이콘] 보내주신 image_5.png의 실루엣을 그대로 재현한 배달 스쿠터 SVG
const DeliveryScooterIcon = ({ className = "" }) => (
  <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    <circle cx="20" cy="50" r="6" stroke="currentColor" strokeWidth="4"/>
    <circle cx="50" cy="50" r="6" stroke="currentColor" strokeWidth="4"/>
    <path d="M10 44H54L46 22H24L10 44Z" fill="currentColor" fillOpacity="0.2" stroke="currentColor" strokeWidth="3" />
    <rect x="6" y="14" width="22" height="18" rx="2" fill="#FBBF24" stroke="currentColor" strokeWidth="2.5"/>
    <circle cx="34" cy="16" r="6" fill="currentColor" />
    <path d="M42 22L46 10H58" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M28 22L30 44" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
  </svg>
);

// [아이콘] 안라무복 낙관(도장) - 호준님의 요청 스타일
...
export default function Page() {
  ...
  const handleSave = async () => {
    if (!formData.name || !formData.password) return alert('내용을 채워주세요!');
    if (editingItem) {
  ...
  const rollback = async (log: any) => {
    if (!confirm('이 시점의 정보로 복구할까요?')) return;
  ...
  // UI 로직: 초기 진입 시(검색어 없고 전체 탭일 때) 리스트를 비워 푸터가 바로 보이게 함
    ...
    <div className="min-h-screen bg-[#070b14] text-white font-sans tracking-tight pb-40 relative overflow-x-hidden">
      ...
      <div className="bg-[#0f172a]/95 border-b border-slate-800/60 sticky top-0 z-40 backdrop-blur-lg shadow-2xl">
        <div className="p-5 flex items-center gap-4 relative z-10">
          <DeliveryScooterIcon className="w-14 h-14 text-yellow-500 drop-shadow-[0_0_10px_rgba(234,179,8,0.3)]" />
          <div className="flex-1">
            <h1 className="text-2xl font-black text-white italic tracking-tighter uppercase">영종도 <span className="text-yellow-400 font-black">배달 라이더</span></h1>
            <p className="text-[10px] text-slate-500 font-bold uppercase mt-1 tracking-widest">공동현관 출입 정보망</p>
          </div>
          <div className="text-[9px] font-bold text-slate-400 bg-slate-900 px-2 py-1 rounded-lg border border-slate-800 flex items-center gap-1.5 shadow-inner">
        ...
        <div className="px-5 pb-5 relative z-10">
          <div className="relative group flex flex-col justify-center">
            <Search className="absolute left-4 text-slate-800 group-focus-within:text-yellow-500 transition-colors z-20" size={20} />
            <input 
              type="text" 
              className="w-full p-4.5 pl-12 bg-[#1e293b] rounded-2xl border border-slate-700/50 focus:border-yellow-500/60 text-lg focus:outline-none shadow-xl transition-all font-bold" 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)} 
            />
            {/* 투명하고 작은 초성 안내 문구 */}
            {!searchTerm && (
              <span className="absolute left-12 text-[13px] text-slate-600 font-bold pointer-events-none opacity-50">
                건물명 초성 검색 가능 (예: ㄱㄹㄷㅂ)
              </span>
            )}
          </div>
          ...
          <div key={i.id} className="bg-[#111827] p-5 rounded-[2.5rem] border border-slate-800/40 shadow-inner">
            <div className="flex justify-between items-start">
              <div className="flex-1 mr-2">
                ...
                <h2 className="text-xl font-bold mt-1 text-white leading-snug tracking-tight break-keep">{i.name}</h2>
              </div>
      ...
      {/* [푸터] 요청하신 모든 사양 통합 */}
      <footer className={`flex flex-col items-center text-center px-10 transition-all duration-700 relative z-10 ${isInitialState ? 'mt-20 opacity-100' : 'mt-10 opacity-30 scale-95'}`}>
        <div className="w-full max-w-sm bg-[#1e293b]/60 backdrop-blur-md p-8 rounded-[3.5rem] border border-slate-800 shadow-2xl flex flex-col items-center gap-6">
          <AnRaMuBokSeal className="w-20 h-20 shadow-2xl shadow-red-900/40" />
          
          <div className="space-y-3">
            <p className="text-[11px] text-yellow-500 font-black tracking-[0.25em] uppercase">STAY ALERT, RIDE SAFE</p>
          ...
          {/* 만든이 가독성 확보 (검정 박스 + 노란 글씨) */}
          <div className="bg-[#070b14] px-6 py-2.5 rounded-2xl border border-slate-800 shadow-inner">
            <p className="text-[13px] text-white font-bold tracking-tight">
      ...
      {/* 플로팅 추가 버튼 */}
      <button onClick={() => {setEditingItem(null); setFormData({ region: '운서', name: '', password: '', note: '' }); setIsModalOpen(true);}} className="fixed bottom-10 right-8 bg-yellow-500 text-black p-5 rounded-2xl shadow-[0_10px_30px_rgba(234,179,8,0.4)] z-50 active:scale-90 transition-all">
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] flex items-center justify-center p-6 text-sm">
          <div className="bg-[#1e293b] w-full max-w-md rounded-[3rem] p-8 border border-slate-800 shadow-3xl text-sm">
            ...
            <div className="space-y-5">
              ...
              <textarea placeholder="특이사항" className="w-full p-4.5 bg-[#070b14] rounded-2xl border border-slate-800 text-white outline-none h-28 shadow-inner placeholder:text-slate-800" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} />

...
export default function Page() {
  ...
  const [activeTab, setActiveTab] = useState('Home'); // 초기 상태 구분용
  ...
  // 핵심 로직: 첫 접속(Home)이거나 검색어가 없는데 '전체' 탭인 경우 리스트를 비움
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white font-sans tracking-tight pb-32 relative overflow-x-hidden">
      
      {/* 배경 이미지: 흐릿하게 배달 관련 감성 삽입 */}
      <div className="fixed inset-0 bg-[url('https://images.unsplash.com/photo-1558981285-6f0c94958bb6?q=80&w=1000&auto=format')] bg-cover bg-center opacity-[0.03] grayscale pointer-events-none"></div>

      {/* 헤더 */}
      <div className="bg-[#0f172a]/95 border-b border-slate-800/60 sticky top-0 z-40 backdrop-blur-lg shadow-2xl">
        <div className="p-5 flex items-center gap-4 relative z-10">
          <DeliveryScooterIcon className="w-14 h-14 text-yellow-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.4)]" />
          ...
          <div className="text-[9px] font-bold text-slate-500 bg-slate-900/80 px-2 py-1 rounded-lg border border-slate-800 flex items-center gap-1.5">
        ...
        <div className="px-5 pb-5 relative z-10">
          <div className="relative group">
            ...
            />
            <Search className="absolute left-5 top-4.5 text-slate-800 group-focus-within:text-yellow-500 transition-colors" size={22} />
          </div>
      ...
      <div className="p-5 space-y-4 min-h-[50px] relative z-10">
        {filtered.map(i => (
          <div key={i.id} className="bg-[#111827] p-5 rounded-[2rem] border border-slate-800/40 shadow-inner">
      ...
      <footer className={`text-center px-10 transition-all duration-500 ${isInitialState ? 'mt-20 opacity-100' : 'mt-10 opacity-50'}`}>
        ...
        <div className="mt-6 pt-6 border-t border-slate-900/50 max-w-[150px] mx-auto">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-[#1e293b] w-full max-w-md rounded-[3rem] p-8 border border-slate-800 shadow-3xl">
            <h3 className="text-2xl font-black text-yellow-400 mb-8 tracking-tighter">{editingItem ? '정보 수정' : '신규 등록'}</h3>
            <div className="space-y-5">
              ...
              <textarea placeholder="특이사항" className="w-full p-4.5 bg-[#070b14] rounded-2xl border border-slate-800 text-white outline-none h-28 placeholder:text-slate-800 shadow-inner text-sm" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} />
              <div className="flex gap-2">
                ...
                <button onClick={handleSave} className="flex-[2] bg-yellow-500 text-black p-5 rounded-2xl font-black text-xl shadow-xl">저장</button>
              </div>
            </div>
          </div>
        </div>

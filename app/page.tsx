...
// [로고 1] 배달 스쿠터 아이콘 (뒤에 배달박스 실루엣 강조)
...
// [로고 2] 요청하신 '안라무복' 낙관 로고 (전통 도장 스타일)
...
export default function Page() {
  ...
  const [activeTab, setActiveTab] = useState('Home'); // 초기 상태 Home
  ...
  // 핵심: 첫 진입(Home)이면서 검색어가 없을 때는 리스트를 비움
  const isInitial = activeTab === 'Home' && searchTerm === '';
  const filtered = isInitial ? [] : data.filter(i => {
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white font-sans tracking-tight pb-32 overflow-x-hidden">
      ...
      <div className="bg-[#0f172a]/95 border-b border-slate-800/60 sticky top-0 z-40 backdrop-blur-lg shadow-2xl">
        <div className="p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">공동현관 출입 정보망</p>
          </div>
          <div className="text-[9px] font-bold text-slate-400 bg-slate-900/80 px-2.5 py-1.5 rounded-lg border border-slate-800 flex items-center gap-1.5">
        ...
        <div className="px-5 pb-5">
          <div className="relative group">
            <input 
              ...
              className="w-full p-4.5 pl-14 bg-[#1e293b] rounded-2xl border border-slate-700/50 focus:border-yellow-500 shadow-xl text-lg font-bold outline-none" 
              onChange={(e) => {
                setSearchTerm(e.target.value);
                if(activeTab === 'Home') setActiveTab('전체');
              }} 
            />
            <Search className="absolute left-5 top-4.5 text-slate-700 group-focus-within:text-yellow-500" size={22} />
          </div>
          <div className="flex gap-2 mt-5 overflow-x-auto no-scrollbar">
            {['전체', '운서', '하늘', '화장실'].map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-5 py-2.5 rounded-xl font-bold text-xs transition-all ${activeTab === t ? 'bg-yellow-500 text-black shadow-lg' : 'bg-slate-800/40 text-slate-600'}`}>
                {t}
              </button>
            ))}
          </div>
      ...
      {/* 리스트 영역 */}
      <div className="p-5 space-y-4 min-h-[50px] relative z-10">
        {filtered.map(i => (
          <div key={i.id} className="bg-[#111827] p-5 rounded-[2.5rem] border border-slate-800/40 shadow-inner">
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-1.5">
                ...
                <button onClick={() => {navigator.clipboard.writeText(i.password); alert('비밀번호 복사 완료');}} className="bg-yellow-500/10 p-3 rounded-xl text-yellow-500 border border-yellow-500/20 active:scale-90 transition-all"><Copy size={18} /></button>
              </div>
            </div>
      ...
      {/* [핵심] 푸터: 초기 화면에서 중앙에 웅장하게 노출 */}
      <footer className={`flex flex-col items-center text-center px-10 transition-all duration-700 ${isInitial ? 'mt-20 opacity-100' : 'mt-10 opacity-30 scale-90'}`}>
        <div className="bg-[#1e293b]/80 backdrop-blur-md p-8 rounded-[3rem] border border-slate-700 shadow-3xl flex flex-col items-center gap-6">
          <AnRaMuBokSeal className="w-24 h-24 shadow-2xl shadow-red-900/50" />
          <div className="space-y-3">
            <p className="text-[10px] text-yellow-500 font-black tracking-[0.3em] uppercase">Ride Safe, Get Home Safe</p>
            <p className="text-[16px] text-white font-black leading-snug tracking-tight">
              오늘도 영종도의 모든 길 위에서<br/>안라무복하시길 기원합니다.
            </p>
            <div className="h-px w-10 bg-slate-700 mx-auto"></div>
            {/* 만든이 글자 가독성 강화 */}
            <p className="text-[13px] text-white font-bold bg-slate-800/80 px-4 py-1.5 rounded-full inline-block border border-slate-700 shadow-inner">
              만든이 : <span className="text-yellow-400 ml-1 font-black">부업맨 HoJun</span>
            </p>
          </div>
        </div>
      </footer>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-[#1e293b] w-full max-w-md rounded-[3rem] p-8 border border-slate-800 shadow-3xl">
            <h3 className="text-2xl font-black text-yellow-400 mb-8">{editingItem ? '정보 수정' : '신규 등록'}</h3>
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-2">
                {['운서', '하늘', '화장실'].map(r => (
                  <button key={r} onClick={() => setFormData({...formData, region: r})} className={`py-3 rounded-2xl font-bold border transition-all text-sm ${formData.region === r ? 'bg-yellow-500 border-yellow-500 text-black' : 'bg-slate-900 border-slate-800 text-slate-600'}`}>{r}</button>
                ))}
              </div>
              ...
              <textarea placeholder="특이사항" className="w-full p-4.5 bg-[#070b14] rounded-2xl border border-slate-800 text-white outline-none h-28 text-sm placeholder:text-slate-800" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} />
              <div className="flex gap-2 mt-4">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 bg-slate-800 p-5 rounded-2xl font-bold">취소</button>
                <button onClick={handleSave} className="flex-[2] bg-yellow-500 text-black p-5 rounded-2xl font-black text-xl">저장하기</button>
              </div>
            </div>
          </div>
        </div>

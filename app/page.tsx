      ...
      <div className="bg-[#0f172a]/95 border-b border-slate-800/60 sticky top-0 z-40 backdrop-blur-lg shadow-2xl">
        <div className="p-3.5 flex items-center justify-between gap-2 relative z-10">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex items-baseline gap-1.5 min-w-0">
              ...
              <span className="text-[9px] text-slate-500 font-bold tracking-tighter truncate">
                공동현관 출입 정보망
              </span>
            </div>
          </div>
          <div className="text-[8px] font-bold text-slate-400 bg-slate-900 px-2 py-1 rounded border border-slate-800 flex items-center gap-1 shrink-0 shadow-inner">
        ...
        <div className="px-5 pb-5 relative z-10">
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto no-scrollbar">
            {['전체', '운서', '하늘', '화장실'].map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-all text-xs ${activeTab === t ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/20' : 'bg-slate-800/40 text-slate-600 border border-slate-700/30'}`}>
                {t}
              </button>
            ))}
          </div>
          ...
          <div key={i.id} className="bg-[#111827] p-5 rounded-[2.5rem] border border-slate-800/40 shadow-inner">
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-1.5">
                <button onClick={() => showHistory(i.id)} className="bg-slate-800/50 p-2.5 rounded-xl text-slate-600 border border-slate-700/20"><History size={16} /></button>
                <button onClick={() => {setEditingItem(i); setFormData({ region: i.region, name: i.name, password: i.password, note: i.note }); setIsModalOpen(true);}} className="bg-slate-800/50 p-2.5 rounded-xl text-slate-600 border border-slate-700/20"><Edit2 size={16} /></button>
          ...
          <div className="bg-[#1e293b] w-full max-w-md rounded-[3rem] p-8 border border-slate-800 shadow-3xl break-keep">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-2">
                {['운서', '하늘', '화장실'].map(r => (
                  <button key={r} onClick={() => setFormData({...formData, region: r})} className={`py-3 rounded-2xl font-bold border transition-all ${formData.region === r ? 'bg-yellow-500 text-black shadow-lg' : 'bg-slate-900/50 text-slate-600 bg-slate-800/50 border-slate-800'}`}>{r}</button>
                ))}
              </div>
              ...
              <div className="flex gap-2.5 mt-4">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 bg-slate-800 p-5 rounded-2xl font-bold active:scale-95 transition-all text-white border border-slate-700/50">Cancel</button>
                <button onClick={handleSave} className="flex-[2] bg-yellow-500 text-black p-5 rounded-2xl font-black text-xl shadow-yellow-500/10 active:scale-95 transition-all">Submit</button>
              </div>
            </div>
          </div>
        </div>
